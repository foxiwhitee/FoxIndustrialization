...
public class C2SClearTankInMTPacket extends BasePacket {
    private final int xCoord, yCoord, zCoord;

    public C2SClearTankInMTPacket(ByteBuf data) {
        super(data);
        xCoord = data.readInt();
        yCoord = data.readInt();
        zCoord = data.readInt();
    ...
    public C2SClearTankInMTPacket(int xCoord, int yCoord, int zCoord) {
        super();
        this.xCoord = xCoord;
        this.yCoord = yCoord;
        this.zCoord = zCoord;
    ...
    public void handleServerSide(IInfoPacket network, BasePacket packet, EntityPlayer player) {
        if (player != null && player.worldObj != null) {
            ...
            if (te instanceof TileMolecularTransformer tile) {
                tile.clearTank();
            }
        }
    }
